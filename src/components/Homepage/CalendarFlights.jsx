...
export default function FlightPicker({ bookingCardRef }) {
  ...
  const fetchCalendar = () => {
    api
      .get(
        `flights/?origin_city=${departureState}&destination_city=${arrivalState}`,
        {
          headers: {
      ...
      .then((response) => {
        ...
        const relevantFlights = response.data.data.filter(
          (flight) => new Date(flight.departure_date) > currentDate
        );
  ...
  const getFlightPriceForDay = (day) => {
    const flightOnDay = flights.find(
      (flight) =>
    ...
    if (flightOnDay) {
      const originalPrice = flightOnDay.price;
      const discountedPrice = flightOnDay.discounted_price;
  ...
  return (
    <>
      ...
      <div className="h-full mx-[13%] p-8 mb-10 bg-white rounded-md shadow-md grid grid-cols-4 gap-3">
        {" "}
        {/* Container */}
        {/* Flight Details */}
        <div className="col-span-1">
          <div className="flex flex-col justify-between mb-4">
            ...
            <div className="flex flex-row">
              ...
              <Button variant="contained" fullWidth onClick={fetchCalendar}>
                Search
              </Button>
            </div>
          </div>
        ...
        <div className="bg-[#F9FAFB] col-span-3 p-8">
          ...
          <div className="grid grid-cols-7 gap-4 mt-8">
            ...
            {Array.from({ length: startDay }).map((_, index) => (
              <div
                key={`prevMonth-${index}`}
                className="border p-4 text-gray-400"
              >
            ...
            {Array.from({ length: monthDays }).map((_, index) => {
              const flightOnDay = flights.find(
                (flight) =>
                  ...
                  new Date(flight.departure_date).getMonth() ===
                    date.getMonth() &&
              ...
              const priceDetails = getFlightPriceForDay(index + 1);
                      ...
                      ? () => {
                          ...
                          dispatch(
                            setFlightDetails({
                              ...
                              discountedPrice: flightOnDay.discounted_price,
